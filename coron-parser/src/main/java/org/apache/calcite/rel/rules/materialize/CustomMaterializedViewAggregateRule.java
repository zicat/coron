 ...
 */
public abstract class CustomMaterializedViewAggregateRule<
                C extends CustomMaterializedViewAggregateRule.Config>
        extends CustomMaterializedViewRule<C> {
    ...
    CustomMaterializedViewAggregateRule(C config) {
        super(config);
    }
    ...
    @Override
    protected @Nullable ViewPartialRewriting compensateViewPartial(
        ...
        final Aggregate newViewNode =
                aggregateViewNode.copy(
                        aggregateViewNode.getTraitSet(),
                        relBuilder.build(),
                        groupSet.build(),
                        null,
        ...
        if (topViewProject != null) {
            ...
            for (int i = aggregateViewNode.getRowType().getFieldCount();
                    i < newViewNode.getRowType().getFieldCount();
                    i++) {
    ...
    @Override
    protected @Nullable RelNode rewriteQuery(
            RelBuilder relBuilder,
            RexBuilder rexBuilder,
    ...
    @Override
    protected @Nullable RelNode createUnion(
            RelBuilder relBuilder,
            RexBuilder rexBuilder,
        ...
        for (int i = 0; i < relBuilder.peek().getRowType().getFieldCount(); i++) {
            ...
            exprList.add(
                    rexBuilder.ensureType(
                            field.getType(), rexBuilder.makeInputRef(relBuilder.peek(), i), true));
        ...
        RelNode result =
                relBuilder.aggregate(relBuilder.groupKey(groupSet), aggregateCalls).build();
    ...
    /**
     * Given a relational expression with a single input (such as a Project or Aggregate) and the
     * ordinal of an input field, returns the ordinal of the output field that references the input
     * field. Or -1 if the field is not propagated.
     *
     * <p>For example, if {@code rel} is {@code Project(c0, c2)} (on input with columns (c0, c1,
     * c2)), then {@code find(rel, 2)} returns 1 (c2); {@code find(rel, 1)} returns -1 (because c1
     * is not projected).
     *
     * <p>If {@code rel} is {@code Aggregate([0, 2], sum(1))}, then {@code find(rel, 2)} returns 1,
     * and {@code find(rel, 1)} returns -1.
     *
        ...
        for (int i = 0; i < target.getRowType().getFieldCount(); i++) {
            ...
            final RexNode expr =
                    RexUtil.swapTableColumnReferences(
                            rexBuilder, simplified, tableMapping.inverse(), equivalenceClassesMap);
        ...
        for (RexNode timestampExpr : timestampExprs) {
            for (TimeUnitRange value : SUPPORTED_DATE_TIME_ROLLUP_UNITS) {
                ...
                for (SqlFunction function : functions) {
                    final RexNode call =
                            rexBuilder.makeCall(
                                    function, timestampExpr, rexBuilder.makeFlag(value));
                    // References self-row
