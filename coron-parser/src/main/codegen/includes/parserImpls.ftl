...
{
   (
       <PROJECTION> SimpleIdentifier() <LPAREN> SqlSelectWithOutFrom() [OrderBy(true)] <RPAREN>
   |
       id = SimpleIdentifier()
       type = DataType()
       nullable = NullableOptDefaultTrue()
       (e = Expression(ExprContext.ACCEPT_SUB_QUERY) | {e = null;})
       {
           list.add(
               SqlDdlNodes.column(s.add(id).end(this), id,
                   type.withNullable(nullable), e, ColumnStrategy.DEFAULT));
       }
   |
       id = SimpleIdentifier()
...
void Engine(): {}{
    [CompoundIdentifier() <EQ> SelectExpression() ]
    [<PARTITION> <BY>  SelectList()]
    [<ORDER> <BY> SelectList()]
    [<TTL> SelectExpression()]
    [CompoundIdentifier() CompoundIdentifier() <EQ> OptionValue() (<COMMA> CompoundIdentifier() <EQ> OptionValue())*]
}
