...
public class PrestoQueryTest extends NodeRelTest {
    ...
    @Test
    public void testJoin6() throws SqlParseException {

        final String sql1 =
                "SELECT *\n"
                        + "FROM (SELECT\n"
                        + "`vid`, CASE WHEN `ver` IN ('2.0.8.2', '2.2.0.20', '2.2.0.27', '2.2.0.27.4', '2.2.0.27.5', '2.2.0.2701', '2.2.0.2703', '2.2.0.29', '2.2.0.60', '2.2.0.61', '2.2.0.62', '2.2.3.20', '2.2.3.201', '2.2.3.24', '2.3.3.110', '2.4.1.20', '2.4.1.42', '2.4.1.43', '2.9.1.42', '2.9.1.43', '2.9.1.44', '2.9.1.45', '2.9.1.46', 'AMG_1.0', 'AMG_1.1', 'AMG_2.0', 'AMG_2.1') AND CAST(`app_type` AS VARCHAR) = 'UNKNOWN' THEN 'unity' ELSE `app_type` END AS `app_type`, ROUND(SUM(`audio`), 0) AS `audio`, ROUND(SUM(`video`), 0) AS `video`, ROUND(SUM(`audio` + `video`), 0) AS `total_usage`\n"
                        + "FROM `report_datahub`.`agora_vid_usage_cube_di`\n"
                        + "WHERE `date_parse`(CAST(`agora_vid_usage_cube_di`.`date` AS VARCHAR), '%Y%m%d') BETWEEN ASYMMETRIC TIMESTAMP '2021-10-01 00:00:00' AND TIMESTAMP '2021-12-31 00:00:00' AND CAST(`agora_vid_usage_cube_di`.`group_mark` AS VARCHAR) = '(product_line,product_type,domain,country,vid,os,ver,app_type)' AND CAST(`agora_vid_usage_cube_di`.`os` AS VARCHAR) = 'MAC'\n"
                        + "GROUP BY `agora_vid_usage_cube_di`.`vid`, CASE WHEN `agora_vid_usage_cube_di`.`ver` IN ('2.0.8.2', '2.2.0.20', '2.2.0.27', '2.2.0.27.4', '2.2.0.27.5', '2.2.0.2701', '2.2.0.2703', '2.2.0.29', '2.2.0.60', '2.2.0.61', '2.2.0.62', '2.2.3.20', '2.2.3.201', '2.2.3.24', '2.3.3.110', '2.4.1.20', '2.4.1.42', '2.4.1.43', '2.9.1.42', '2.9.1.43', '2.9.1.44', '2.9.1.45', '2.9.1.46', 'AMG_1.0', 'AMG_1.1', 'AMG_2.0', 'AMG_2.1') AND CAST(`agora_vid_usage_cube_di`.`app_type` AS VARCHAR) = 'UNKNOWN' THEN 'unity' ELSE `agora_vid_usage_cube_di`.`app_type` END) AS `t1`\n"
                        + "INNER JOIN (SELECT `vid`, `companyid`, `companyname`, `projectname`, `country`, `is_explore`, `area`\n"
                        + "FROM `report_datahub`.`new_vendor_dimension`\n"
                        + "WHERE 1 = 1) AS `t2` ON CAST(`t1`.`vid` AS VARCHAR) = CAST(`t2`.`vid` AS VARCHAR)\n"
                        + "WHERE 1 = 1 AND CAST(`t1`.`app_type` AS VARCHAR) <> 'UNKNOWN' AND CAST(`t1`.`app_type` AS VARCHAR) = 'flutter'";
        final String sql2 =
                "SELECT *\n"
                        + "FROM (SELECT\n"
                        + "    vid,\n"
