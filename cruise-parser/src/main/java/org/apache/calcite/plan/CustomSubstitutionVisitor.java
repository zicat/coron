 ...
 *
 * <p>The call {@code new SubstitutionVisitor(target, query).go(replacement))} will return {@code
 * query} with every occurrence of {@code target} replaced by {@code replacement}.
 *
 * <p>The following example shows how {@code SubstitutionVisitor} can be used for materialized view
 * recognition.
 *
 * <ul>
 *   <li>query = SELECT a, c FROM t WHERE x = 5 AND b = 4
 *   <li>target = SELECT a, b, c FROM t WHERE x = 5
 *   <li>replacement = SELECT * FROM mv
 *   <li>result = SELECT a, c FROM mv WHERE b = 4
 * </ul>
 *
 * <p>Note that {@code result} uses the materialized view table {@code mv} and a simplified
 * condition {@code b = 4}.
 *
 * <p>Uses a bottom-up matching algorithm. Nodes do not need to be identical. At each level, returns
 * the residue.
 *
 * <p>The inputs must only include the core relational operators: {@link TableScan}, {@link Filter},
 * {@link Project}, {@link Calc}, {@link Join}, {@link Union}, {@link Intersect}, {@link Aggregate}.
 */
public class CustomSubstitutionVisitor {
    ...
    /**
     * Workspace while rule is being matched. Careful, re-entrant! Assumes no rule needs more than 2
     * slots.
     */
    protected final MutableRel[] slots = new MutableRel[2];

    /** Creates a CustomSubstitutionVisitor with the default rule set. */
    public CustomSubstitutionVisitor(RelNode target_, RelNode query_) {
        this(target_, query_, DEFAULT_RULES, RelFactories.LOGICAL_BUILDER);
    }

    /** Creates a CustomSubstitutionVisitor with the default logical builder. */
    public CustomSubstitutionVisitor(
            RelNode target_, RelNode query_, ImmutableList<UnifyRule> rules) {
     ...
     *
     * <p>If condition is stronger than target, returns the residue. If it is equal to target,
     * returns the expression that evaluates to the constant {@code true}. If it is weaker than
     * target, returns {@code null}.
     *
     ...
     *
     * <ul>
     *   <li>condition: x = 1 AND y = 2
     *   <li>target: x = 1
     *   <li>residue: y = 2
     * </ul>
     *
     * <p>Note that residue {@code x > 0 AND y = 2} would also satisfy the relation {@code condition
     * = target AND residue} but is stronger than necessary, so we prefer {@code y = 2}.
     *
     * <p>Example #2: target weaker than condition (valid, but not currently implemented)
     *
     * <ul>
     *   <li>condition: x = 1
     *   <li>target: x = 1 OR z = 3
     *   <li>residue: x = 1
     * </ul>
     *
     ...
     * <ul>
     *   <li>condition: x = 1
     *   <li>target: x = 1 AND y = 2
     *   <li>residue: null (i.e. no match)
     * </ul>
     *
     * <p>There are many other possible examples. It amounts to solving whether {@code condition AND
     * NOT target} can ever evaluate to true, and therefore is a form of the NP-complete <a
     * href="http://en.wikipedia.org/wiki/Satisfiability">Satisfiability</a> problem.
     */
    @VisibleForTesting
    public static @Nullable RexNode splitFilter(
            final RexSimplify simplify, RexNode condition, RexNode target) {
    ...
    /**
     * Reorders some of the operands in this expression so structural comparison, i.e., based on
     * string representation, can be more precise.
     */
            ...
            case OR:
                {
                    ...
                    Map<RexNode, List<Integer>> sorted = new HashMap<>();
                    for (RexNode operand : call.operands) {
                        List<Integer> predicts = PredictRexShuttle.predicts(operand);
                        operand = canonizeNode(rexBuilder, operand);
                        newOperands.add(operand);
                        sorted.put(operand, predicts);
                    }
                    ...
                    newOperands.sort(
                            (o1, o2) -> {
                                List<Integer> predict = sorted.get(o1);
                                List<Integer> predict2 = sorted.get(o2);
                                int minSize = Math.min(predict.size(), predict2.size());
                                for (int i = 0; i < minSize; i++) {
                                    if (Objects.equals(predict.get(i), predict2.get(i))) {
                                        continue;
                                    }
                                    return predict.get(i).compareTo(predict2.get(i));
                                }
                                return 0;
                            });
                    return rexBuilder.makeCall(
                            call.getOperator(), ImmutableList.copyOf(newOperands));
                }
            case EQUALS:
            case NOT_EQUALS:
            case LESS_THAN:
     ...
     *
     * <p>This method may give false positives. For instance, it will say that {@code x = 5 AND x >
     * 10} is satisfiable, because at present it cannot prove that it is not.
     */
    ...
    /**
     * Returns a list of all possible rels that result from substituting the matched RelNode with
     * the replacement RelNode within the query.
     *
     * <p>For example, the substitution result of A join B, while A and B are both a qualified match
     * for replacement R, is R join B, R join R, A join R.
     */
    ...
    /**
     * Substitutes the query with replacement whenever possible but meanwhile keeps track of all the
     * substitutions and their original rel before replacement, so that in later processing stage,
     * the replacement can be recovered individually to produce a list of all possible rels with
    ...
    private List<List<Replacement>> go(MutableRel replacement) {
        ...
        for (MutableRel targetDescendant : targetDescendants) {
            ...
            if (queryDescendant != null) {
                assert rowTypesAreEquivalent(queryDescendant, targetDescendant, Litmus.THROW);
                        ...
                        if (call != null) {
                            ...
                            if (result != null) {
                                ...
                                assert rowTypesAreEquivalent(
                                        result.result, result.call.query, Litmus.THROW);
    ...
    private static boolean rowTypesAreEquivalent(MutableRel rel0, MutableRel rel1, Litmus litmus) {
    ...
    /**
     * Represents a replacement action: before &rarr; after. {@code stopTrying} indicates whether
     * there's no need to do matching for the same query node again.
     */
     ...
     * {@code replace}.
     *
     * <p>Assumes relational expressions (and their descendants) are not null. Does not handle
     * cycles.
     */
    public static @Nullable Replacement replace(
            MutableRel query, MutableRel find, MutableRel replace) {
    ...
    private static @Nullable Replacement replaceRecurse(
            MutableRel query, MutableRel find, MutableRel replace) {
    ...
    private static void reverseSubstitute(
            RelBuilder relBuilder,
            Holder query,
            List<List<Replacement>> matches,
            List<RelNode> sub,
            int replaceCount,
            int maxCount) {
        if (matches.isEmpty()) {
    ...
    private @Nullable UnifyResult apply(UnifyRule rule, MutableRel query, MutableRel target) {
        final UnifyRuleCall call = new UnifyRuleCall(rule, query, target, ImmutableList.of());
    ...
    /**
     * Rule that attempts to match a query relational expression against a target relational
     * expression.
     *
     * <p>The rule declares the query and target types; this allows the engine to fire only a few
    ...
    protected abstract static class UnifyRule {
        ...
        /**
         * Applies this rule to a particular node in a query. The goal is to convert {@code query}
         * into {@code target}. Before the rule is invoked, Calcite has made sure that query's
         * children are equivalent to target's children.
         *
         * <p>There are 3 possible outcomes:
         *
         * <ul>
         *   <li>{@code query} already exactly matches {@code target}; returns {@code target}
         *   <li>{@code query} is sufficiently close to a match for {@code target}; returns {@code
         *       target}
         *   <li>{@code query} cannot be made to match {@code target}; returns null
         * </ul>
         *
         * <p>REVIEW: Is possible that we match query PLUS one or more of its ancestors?
         *
         * @param call Input parameters
         */
        protected abstract @Nullable UnifyResult apply(UnifyRuleCall call);

        protected @Nullable UnifyRuleCall match(
                CustomSubstitutionVisitor visitor, MutableRel query, MutableRel target) {
            if (queryOperand.matches(visitor, query)) {
                if (targetOperand.matches(visitor, target)) {
                    return visitor
                    .new UnifyRuleCall(this, query, target, copy(visitor.slots, slotCount));
                }
            }
            return null;
        }
    ...
    /**
     * Result of an application of a {@link UnifyRule} indicating that the rule successfully matched
     * {@code query} against {@code target} and generated a {@code result} that is equivalent to
     * {@code query} and contains {@code target}. {@code stopTrying} indicates whether there's no
     * need to do matching for the same query node again.
     */
    ...
    protected abstract static class AbstractUnifyRule extends UnifyRule {
        ...
        protected AbstractUnifyRule(Operand queryOperand, Operand targetOperand, int slotCount) {
        ...
        protected static Operand operand(
                Class<? extends MutableRel> clazz, Operand... inputOperands) {
     ...
     *
     * <p>Matches scans to the same table, because these will be {@link MutableScan}s with the same
     * {@link TableScan} instance.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableScan} to a {@link MutableCalc} which has
     * {@link MutableScan} as child.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableCalc} to a {@link MutableCalc}. The matching
     * condition is as below: 1. All columns of query can be expressed by target; 2. The filtering
     * condition of query must equals to or be weaker than target.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableJoin} which has {@link MutableCalc} as left
     * child to a {@link MutableJoin}. We try to pull up the {@link MutableCalc} to top of {@link
     * MutableJoin}, then match the {@link MutableJoin} in query to {@link MutableJoin} in target.
     */
    private static class JoinOnLeftCalcToJoinUnifyRule extends AbstractUnifyRule {
        ...
        private JoinOnLeftCalcToJoinUnifyRule() {
            super(
                    ...
                    operand(MutableJoin.class, target(0), target(1)),
                    2);
        }
        ...
        protected @Nullable UnifyResult apply(UnifyRuleCall call) {
            ...
            final List<RexNode> identityProjects = rexBuilder.identityProjects(qInput1.rowType);
            if (!referenceByMapping(query.condition, qInput0Projs, identityProjects)) {
                return null;
            }

            final RexNode newQueryJoinCond =
                    new RexShuttle() {
                        @Override
            ...
            if (splitted != null && splitted.isAlwaysTrue()) {
                ...
                for (int i = 0; i < fieldCnt(query); i++) {
                    if (i < fieldCnt(qInput0)) {
                        ...
                        compenProjs.add(
                                new RexInputRef(
                                        newIdx, query.rowType.getFieldList().get(i).getType()));
                    }
                }
                final RexProgram compenRexProgram =
                        RexProgram.create(
                                target.rowType, compenProjs, compenCond, query.rowType, rexBuilder);
                final MutableCalc compenCalc = MutableCalc.of(target, compenRexProgram);
                return tryMergeParentCalcAndGenResult(call, compenCalc);
            }
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableJoin} which has {@link MutableCalc} as right
     * child to a {@link MutableJoin}. We try to pull up the {@link MutableCalc} to top of {@link
     * MutableJoin}, then match the {@link MutableJoin} in query to {@link MutableJoin} in target.
     */
    private static class JoinOnRightCalcToJoinUnifyRule extends AbstractUnifyRule {
        ...
        protected @Nullable UnifyResult apply(UnifyRuleCall call) {
            ...
            // Try pulling up MutableCalc only when Join condition references mapping.
            final List<RexNode> identityProjects = rexBuilder.identityProjects(qInput0.rowType);
                    ...
                    new RexShuttle() {
                        ...
                        public RexNode visitInputRef(RexInputRef inputRef) {
                            ...
                            if (idx < fieldCnt(qInput0)) {
                                ...
                                final int newIdx =
                                        ((RexInputRef) qInput1Projs.get(idx - fieldCnt(qInput0)))
                                                        .getIndex()
                                                + fieldCnt(qInput0);
                                return new RexInputRef(newIdx, inputRef.getType());
                            }
                ...
                for (int i = 0; i < query.rowType.getFieldCount(); i++) {
                    if (i < fieldCnt(qInput0)) {
                        ...
                        final RexNode shifted =
                                RexUtil.shift(
                                        qInput1Projs.get(i - fieldCnt(qInput0)),
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableJoin} which has {@link MutableCalc} as
     * children to a {@link MutableJoin}. We try to pull up the {@link MutableCalc} to top of {@link
     * MutableJoin}, then match the {@link MutableJoin} in query to {@link MutableJoin} in target.
     */
    private static class JoinOnCalcsToJoinUnifyRule extends AbstractUnifyRule {
        ...
        private JoinOnCalcsToJoinUnifyRule() {
            super(
                    operand(
                            MutableJoin.class,
                            operand(MutableCalc.class, query(0)),
            ...
            if (splitted != null && splitted.isAlwaysTrue()) {
                ...
                final RexNode compenCond =
                        RexUtil.composeConjunction(
                                rexBuilder, ImmutableList.of(qInput0Cond, qInput1CondShifted));
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableAggregate} which has {@link MutableCalc} as
     * child to a {@link MutableAggregate}. We try to pull up the {@link MutableCalc} to top of
     * {@link MutableAggregate}, then match the {@link MutableAggregate} in query to {@link
     * MutableAggregate} in target.
     */
    private static class AggregateOnCalcToAggregateUnifyRule extends AbstractUnifyRule {
        ...
        private AggregateOnCalcToAggregateUnifyRule() {
            super(
                    operand(MutableAggregate.class, operand(MutableCalc.class, query(0))),
                    operand(MutableAggregate.class, target(0)),
                    1);
        }
        ...
        protected @Nullable UnifyResult apply(UnifyRuleCall call) {
                            ...
                            new RexVisitorImpl<Void>(true) {
                                ...
                                public Void visitInputRef(RexInputRef inputRef) {
                                    if (!target.groupSets.stream()
                                            .allMatch(
                                                    groupSet ->
                                                            groupSet.get(inputRef.getIndex()))) {
            ...
            final MutableAggregate aggregate2 = permute(query, qInput.getInput(), inverseMapping);
    ...
    /**
     * A {@link UnifyRule} that matches a {@link Aggregate} to a {@link Aggregate}, provided that
     * they have the same child.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableUnion} to a {@link MutableUnion} where the
     * query and target have the same inputs but might not have the same order.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableUnion} which has {@link MutableCalc} as
     * child to a {@link MutableUnion}. We try to pull up the {@link MutableCalc} to top of {@link
     * MutableUnion}, then match the {@link MutableUnion} in query to {@link MutableUnion} in
     * target.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableIntersect} to a {@link MutableIntersect}
     * where the query and target have the same inputs but might not have the same order.
     */
    ...
    /**
     * A {@link UnifyRule} that matches a {@link MutableIntersect} which has {@link MutableCalc} as
     * child to a {@link MutableIntersect}. We try to pull up the {@link MutableCalc} to top of
     * {@link MutableIntersect}, then match the {@link MutableIntersect} in query to {@link
     * MutableIntersect} in target.
     */
    ...
    /**
     * Applies a AbstractUnifyRule to a particular node in a query. We try to pull up the {@link
     * MutableCalc} to top of {@link MutableUnion} or {@link MutableIntersect}, this method not suit
     * for {@link MutableMinus}.
     *
     * @param call Input parameters
     */
    private static @Nullable UnifyResult setOpApply(UnifyRuleCall call) {
        if (call.query instanceof MutableMinus && call.target instanceof MutableMinus) {
            return null;
        }
        ...
        if (query.isAll()
            ...
            for (int i = 1; i < queryGrandInputs.size(); i++) {
                ...
                for (Pair<RexNode, RexNode> pair :
                        Pair.zip(queryInputExplained0.right, queryInputExplained.right)) {
    ...
    /**
     * Generate result by merging parent and child if they are both MutableCalc. Otherwise result is
     * the child itself.
     */
    ...
    private static @Nullable MutableCalc mergeCalc(
        ...
        RexProgram mergedProgram =
                RexProgramBuilder.mergePrograms(topCalc.program, bottomCalc.program, rexBuilder);
    ...
    private static boolean implies(
        ...
        RexExecutor rexImpl = Util.first(cluster.getPlanner().getExecutor(), RexUtil.EXECUTOR);
    ...
    public static MutableAggregate permute(
            MutableAggregate aggregate, MutableRel input, Mapping mapping) {
    ...
    public static @Nullable MutableRel unifyAggregates(
        ...
        Map<RexNode, RexNode> targetCondConstantMap =
                RexUtil.predicateConstants(
                        RexNode.class, rexBuilder, RelOptUtil.conjunctions(targetCond));
        ...
        if (query.groupSets.equals(target.groupSets)
            ...
            for (AggregateCall aggregateCall : query.aggCalls) {
                ...
                // When an SqlAggFunction does not support roll up, it will return null, which means
                // that
    ...
    /**
     * Builds a shuttle that stores a list of expressions, and can map incoming expressions to
     * references to them.
     */
    ...
    public static boolean equalType(
            String desc0, MutableRel rel0, String desc1, MutableRel rel1, Litmus litmus) {
    ...
    protected abstract static class Operand {
        ...
        public abstract boolean matches(CustomSubstitutionVisitor visitor, MutableRel rel);

        public boolean isWeaker(CustomSubstitutionVisitor visitor, MutableRel rel) {
            return false;
        }
    }

    /**
     * Operand to a {@link UnifyRule} that matches a relational expression of a given type. It has
     * zero or more child operands.
     */
    private static class InternalOperand extends Operand {
        ...
        @Override
        public boolean matches(CustomSubstitutionVisitor visitor, MutableRel rel) {
        ...
        private static boolean allMatch(
                CustomSubstitutionVisitor visitor, List<Operand> operands, List<MutableRel> rels) {
     ...
     *
     * <p>It is applied to a descendant of the query, writes the operand into the slots array, and
     * always matches. There is a corresponding operand of type {@link TargetOperand} that checks
     * whether its relational expression, a descendant of the target, is equivalent to this {@code
     * QueryOperand}'s relational expression.
     */
    ...
    /**
     * Operand that checks that a relational expression matches the corresponding relational
     * expression that was passed to a {@link QueryOperand}.
     */
    ...
    /**
     * Visitor that counts how many {@link QueryOperand} and {@link TargetOperand} in an operand
     * tree.
     */
