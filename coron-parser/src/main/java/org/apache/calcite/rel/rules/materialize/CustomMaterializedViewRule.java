...
/**
 * Planner rule that converts a {@link Project} followed by {@link Aggregate} or an {@link
 * Aggregate} to a scan (and possibly other operations) over a materialized view.
 *
 * @param <C> Configuration type
 */
public abstract class CustomMaterializedViewRule<C extends MaterializedViewRule.Config>
        extends RelRule<C> {

    // ~ Constructors -----------------------------------------------------------

    /** Creates a MaterializedViewRule. */
    CustomMaterializedViewRule(C config) {
        super(config);
    }
    ...
    /**
     * Rewriting logic is based on "Optimizing Queries Using Materialized Views: A Practical,
     * Scalable Solution" by Goldstein and Larson.
     *
     * <p>On the query side, rules matches a Project-node chain or node, where node is either an
     * Aggregate or a Join. Subplan rooted at the node operator must be composed of one or more of
     * the following operators: TableScan, Project, Filter, and Join.
     *
     * <p>For each join MV, we need to check the following:
     *
     * <ol>
     *   <li>The plan rooted at the Join operator in the view produces all rows needed by the plan
     *       rooted at the Join operator in the query.
     *   <li>All columns required by compensating predicates, i.e., predicates that need to be
     *       enforced over the view, are available at the view output.
     *   <li>All output expressions can be computed from the output of the view.
     *   <li>All output rows occur with the correct duplication factor. We might rely on existing
     *       Unique-Key - Foreign-Key relationships to extract that information.
     * </ol>
     *
     * <p>In turn, for each aggregate MV, we need to check the following:
     *
     * <ol>
     *   <li>The plan rooted at the Aggregate operator in the view produces all rows needed by the
     *       plan rooted at the Aggregate operator in the query.
     *   <li>All columns required by compensating predicates, i.e., predicates that need to be
     *       enforced over the view, are available at the view output.
     *   <li>The grouping columns in the query are a subset of the grouping columns in the view.
     *   <li>All columns required to perform further grouping are available in the view output.
     *   <li>All columns required to compute output expressions are available in the view output.
     * </ol>
     *
     * <p>The rule contains multiple extensions compared to the original paper. One of them is the
     * possibility of creating rewritings using Union operators, e.g., if the result of a query is
     * partially contained in the materialized view.
     */
            ...
            for (RelOptMaterialization materialization : materializations) {
                ...
                if (!queryTableRefs.equals(viewTableRefs)) {
                    ...
                    if (viewTableRefs.containsAll(queryTableRefs)) {
                        ...
                        if (!compensatePartial(
                                viewTableRefs, vEC, queryTableRefs, compensationEquiColumns)) {
                ...
                for (RelTableRef queryTableRef1 : queryTableRefs) {
                    for (RelTableRef queryTableRef2 : queryTableRefs) {
                        if (queryTableRef1
                                .getQualifiedName()
                                .equals(queryTableRef2.getQualifiedName())) {
                ...
                for (BiMap<RelTableRef, RelTableRef> queryToViewTableMapping : flatListMappings) {
                    ...
                    if (compensationPreds == null && config.generateUnionRewriting()) {
                        ...
                        if (!viewCompensationPred.isAlwaysTrue()) {
                            ...
                            // rewriting.
                            // This is useful in case some of the columns can be folded to same
                            // value after
                            // filter is added.
                            Pair<@Nullable RelNode, RelNode> pushedNodes =
                                    pushFilterToOriginalViewPlan(
                                            builder, topViewProject, viewNode, newPred);
    ...
    protected abstract boolean isValidPlan(
            @Nullable Project topProject, RelNode node, RelMetadataQuery mq);

    /**
     * It checks whether the query can be rewritten using the view even though the query uses
     * additional tables.
     *
    ...
    /**
     * If the view will be used in a union rewriting, this method is responsible for rewriting the
     * query branch of the union using the given compensation predicate.
     *
     * <p>If a rewriting can be produced, we return that rewriting. If it cannot be produced, we
     * will return null.
     */
    ...
    /**
     * If the view will be used in a union rewriting, this method is responsible for generating the
     * union and any other operator needed on top of it, e.g., a Project operator.
     */
    protected abstract @Nullable RelNode createUnion(
            RelBuilder relBuilder,
            RexBuilder rexBuilder,
     ...
     *
     * <p>The input node is a Scan on the view table and possibly a compensation Filter on top. If a
     * rewriting can be produced, we return that rewriting. If it cannot be produced, we will return
     * null.
     */
    protected abstract @Nullable RelNode rewriteView(
            RelBuilder relBuilder,
            RexBuilder rexBuilder,
            RexSimplify simplify,
            RelMetadataQuery mq,
    ...
    /**
     * Once we create a compensation predicate, this method is responsible for pushing the resulting
     * filter through the view nodes. This might be useful for rewritings containing Aggregate
     * operators, as some of the grouping columns might be removed, which results in additional
     * matching possibilities.
     *
     * <p>The method will return a pair of nodes: the new top project on the left and the new node
     * on the right.
     */
     ...
     * Otherwise, it returns a list of references to all columns in the node. The returned list is
     * immutable.
     */
    ...
    /**
     * It will flatten a multimap containing table references to table references, producing all
     * possible combinations of mappings. Each of the mappings will be bi-directional.
     */
    protected List<BiMap<RelTableRef, RelTableRef>> generateTableMappings(
        ...
        for (Map.Entry<RelTableRef, Collection<RelTableRef>> e :
    ...
    /**
     * Returns whether a RelNode is a valid tree. Currently we only support TableScan - Project -
     * Filter - Inner Join.
     */
     ...
     * </ul>
     *
     * <p>For each category, it creates the conjunction of the predicates. The result is an pair of
     * RexNode objects corresponding to each category.
     */
    ...
    /**
     * It checks whether the target can be rewritten using the source even though the source uses
     * additional tables. In order to do that, we need to double-check that every join that exists
     * in the source and is not in the target is a cardinality-preserving join, i.e., it only
     * appends columns to the row without changing its multiplicity. Thus, the join needs to be:
     *
     * <ul>
     *   <li>Equi-join
     *   <li>Between all columns in the keys
     *   <li>Foreign-key columns do not allow NULL values
     *   <li>Foreign-key
     *   <li>Unique-key
     * </ul>
     *
     * <p>If it can be rewritten, it returns true. Further, it inserts the missing equi-join
     * predicates in the input {@code compensationEquiColumns} multimap if it is provided. If it
     * cannot be rewritten, it returns false.
     */
            ...
            for (RelTableRef tRef : graph.vertexSet()) {
                if (graph.getInwardEdges(tRef).size() == 1
                    ...
                    if (compensationEquiColumns != null && extraTableRefs.contains(tRef)) {
                        ...
                        // query
                        compensationEquiColumns.putAll(
                                graph.getInwardEdges(tRef).get(0).equiColumns);
                    }
                }
            }
     ...
     * The method treats separately the equi-column predicates, the range predicates, and the rest
     * of predicates.
     *
     * <p>If the containment is confirmed, we produce compensation predicates that need to be added
     * to the target to produce the results in the source. Thus, if source and target expressions
     * are equivalent, those predicates will be the true constant.
     *
    ...
    protected @Nullable Pair<RexNode, RexNode> computeCompensationPredicates(
        ...
        final RexNode viewPred =
                RexUtil.swapTableColumnReferences(
                        ...
                        sourceEC.getEquivalenceClassesMap());
        compensationPred = CustomSubstitutionVisitor.splitFilter(simplify, queryPred, viewPred);
    ...
    /**
     * Given the equi-column predicates of the source and the target and the computed equivalence
     * classes, it extracts possible mappings between the equivalence classes.
     *
     * <p>If there is no mapping, it returns null. If there is a exact match, it will return a
     * compensation predicate that evaluates to true. Finally, if a compensation predicate needs to
     * be enforced on top of the target to make the equivalences classes match, it returns that
     * compensation predicate.
     */
    protected @Nullable RexNode generateEquivalenceClasses(
            RexBuilder rexBuilder, EquivalenceClasses sourceEC, EquivalenceClasses targetEC) {
            ...
            if (!mapping.containsKey(i)) {
                ...
                while (it.hasNext()) {
                    ...
                    compensationPredicate =
                            rexBuilder.makeCall(
                                    SqlStdOperatorTable.AND, compensationPredicate, equals);
                }
            } else {
    ...
    /**
     * Given the source and target equivalence classes, it extracts the possible mappings from each
     * source equivalence class to each target equivalence class.
     *
     * <p>If any of the source equivalence classes cannot be mapped to a target equivalence class,
     * it returns null.
     */
    ...
    /**
     * First, the method takes the node expressions {@code nodeExprs} and swaps the table and column
     * references using the table mapping and the equivalence classes. If {@code swapTableColumn} is
     * true, it swaps the table reference and then the column reference, otherwise it swaps the
     * column reference and then the table reference.
     *
    ...
    protected @Nullable RexNode rewriteExpression(
        ...
        List<RexNode> rewrittenExprs =
                rewriteExpressions(
                        ...
                        ec,
                        swapTableColumn,
                        ImmutableList.of(exprToRewrite));
     ...
     * {@link RexTableInputRef} by references to the positions in {@code nodeExprs}.
     *
     * <p>The method will return the rewritten expressions. If any of the subexpressions in the
     * input expressions cannot be mapped, it will return null.
     */
        ...
        if (swapTableColumn) {
            nodeLineage =
                    generateSwapTableColumnReferencesLineage(
                            rexBuilder, mq, node, tableMapping, ec, nodeExprs);
        } else {
            nodeLineage =
    ...
    /**
     * It swaps the table references and then the column references of the input expressions using
     * the table mapping and the equivalence classes.
     */
    ...
    /**
     * It swaps the column references and then the table references of the input expressions using
     * the equivalence classes and the table mapping.
     */
    protected NodeLineage generateSwapColumnTableReferencesLineage(
        ...
        for (int i = 0; i < nodeExprs.size(); i++) {
            ...
            final RexNode e =
                    RexUtil.swapColumnTableReferences(
                            rexBuilder, node2, ec.getEquivalenceClassesMap(), tableMapping);
            exprsLineage.putIfAbsent(e, i);
            if (RexUtil.isLosslessCast(e)) {
                exprsLineageLosslessCasts.put(((RexCall) e).getOperands().get(0), i);
            }
    ...
    /**
     * Given the input expression, it will replace (sub)expressions when possible using the content
     * of the mapping. In particular, the mapping contains the digest of the expression and the
     * index that the replacement input ref should point to.
     */
    protected RexNode replaceWithOriginalReferences(
    ...
    /**
     * Replaces all the input references by the position in the input column set. If a reference
     * index cannot be found in the input set, then we return null.
     */
    protected @Nullable RexNode shuttleReferences(
            final RexBuilder rexBuilder, final RexNode node, final Mapping mapping) {
        try {
            RexShuttle visitor =
                    new RexShuttle() {
                        @Override
    ...
    /**
     * Replaces all the possible sub-expressions by input references to the input node. If
     * available, it uses the rewriting mapping to change the position to reference. Takes the
     * reference type from the input node.
     */
    protected @Nullable RexNode shuttleReferences(
            final RexBuilder rexBuilder,
            final RexNode expr,
            final Multimap<RexNode, Integer> exprsLineage,
            final @Nullable RelNode node,
    ...
    protected static class NodeLineage {
        ...
        private NodeLineage(
                Map<RexNode, Integer> exprsLineage,
    ...
    public interface Config extends RelRule.Config {
        /**
         * Whether to generate rewritings containing union if the query results are contained within
         * the view results.
         */
        ...
        /**
         * If we generate union rewriting, we might want to pull up projections from the query
         * itself to maximize rewriting opportunities.
         */
        @ImmutableBeans.Property
